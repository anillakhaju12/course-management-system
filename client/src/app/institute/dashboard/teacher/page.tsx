...
import AddTeacherPopUp from "@/src/lib/components/addTeacherPopUp";
import { useAppDispatch, useAppSelector } from "@/src/lib/store/customHooks";
import { deleteInstituteTeacher } from "@/src/lib/store/institute/teacher/instituteTeacherSlice";
...
export default function InstituteTeacher() {
  ...
  const [isAddTeacherBoxOpen, setIsAddTeacherBoxOpen] = useState<boolean>(false)
  const {teacher} = useAppSelector((store) => store.teacher)
  const {status} = useAppSelector(store => store.courses)


    const openCreateTeacherBox = ()=>{
      setIsAddTeacherBoxOpen(true)
    }
    const closeCreateTeacherBox = ()=>{
    setIsAddTeacherBoxOpen(false)
  }

  const handleDeleteTeacher = (id : string)=>{
    dispatch(deleteInstituteTeacher(id))
  }
  ...
  const filteredData = teacher.filter((teacher)=>teacher.teacherName.includes(searchData))
  return (

    <div>
      {isAddTeacherBoxOpen && <AddTeacherPopUp closeCreateCousrseBox = {closeCreateTeacherBox} />}
          ...
          <div className="min-w-full inline-block align-middle">
            <div className="relative  text-gray-500 focus-within:text-gray-900 mb-4">
              <div className="flex justify-between">
                ...
                />
                <button
                  onClick={openCreateTeacherBox}
                  className="px-2 py-3 rounded text-white bg-teal-600 hover:bg-teal-800"
                >
                  Add Category
                </button>
              </div>
                        ...
                        >
                          ...
                          <td className=" p-5 ">
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={()=>handleDeleteTeacher(course?.courseId)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
                                <svg
                                  width={20}
                                  height={20}
                                  viewBox="0 0 20 20"
